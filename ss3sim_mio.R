...
temp_path <- here()# file.path(tempdir(), "ss3sim-example")
dir.create(temp_path, showWarnings = FALSE)
wd <- getwd()
setwd(wd)#temp_path)
on.exit(setwd(wd), add = TRUE)

# Find the data in the ss3sim folder ...package:
d <- wd# system.file("extdata", package = "ss3sim")
om <- file.path(d, "models2", "mio-om")
em <- file.path(d, "models2", "mio-em")
case_folder <- file.path(d, "eg-cases2")
#

# An example specifying the case files:
run_ss3sim(iterations = 2, scenarios = "D10-F0-E10-mio",
           case_folder = case_folder, om_dir = om, em_dir = em,
           case_files = list(F = "F", D = c("index", "lcomp",
                                            "agecomp"), E = "E"))
unlink("D1-F0-E1-mio", recursive = TRUE) # clean up
